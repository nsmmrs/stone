...
class FileTarget(object):
    ...
    :
        ...
        """
                :param str path: Path from root. Should be an empty string for
                root.
...
class UploadSessionStart(object):
    ...
    :
        ...
        """
                :param str upload_id: A unique identifier for the upload
                session.
        """
...
class UploadAppend(object):
    ...
    :
        ...
        """
                :param str upload_id: Identifies the upload session to append
                data to.
                :param long offset: The offset into the file of the current
                chunk of data being uploaded. It can also be thought of as the
                amount of data that has been uploaded so far. We use the offset
                as a sanity check.
        """
        assert isinstance(upload_id, six.string_types), 'upload_id must be of type six.string_types'
        self.upload_id = upload_id
...
class UploadAppendError(object):
    ...
    :
        ...
        Only one argument can be set.
                ...
                :param None not_found: ``upload_id`` was not found.
                :param None closed: Upload session was closed.
...
class ConflictPolicy(object):
    ...
    The action to take when a file path conflict exists.
            ...
            :ivar Update: On a conflict, only overwrite the target if the
            parent_rev matches.
    """
    ...
    :
        ...
        Only one argument can be set.
                ...
                :param None overwrite: On a conflict, the target is overridden.
                :param None add: On a conflict, the upload is rejected. You can
                call the :meth:`upload` endpoint again and attempt a different
                path.
...
class UploadCommit(object):
    ...
    :
        ...
        """
                ...
                :param mode: The course of action to take if a file already
                exists at ``path``.
        :type mode: :class:`ConflictPolicy`
                :param Nullable append_to: If specified, the current chunk of
                data should be appended to an existing upload session.
                :param bool autorename: Whether the file should be autorenamed
                in the event of a conflict.
                :param Nullable client_modified_utc: Self reported time of when
                this file was created or modified.
                :param bool mute: Whether the devices that the user has linked
                should notify them of the new or updated file.
        """
        assert isinstance(path, six.string_types), 'path must be of type six.string_types'
        self.path = path

        if isinstance(mode, ConflictPolicy.Overwrite):
            self.mode = ConflictPolicy(overwrite=mode)
        if isinstance(mode, ConflictPolicy.Add):
            self.mode = ConflictPolicy(add=mode)
        if isinstance(mode, ConflictPolicy.Update):
            self.mode = ConflictPolicy(update=mode)

        if append_to is not None:
            assert isinstance(append_to, Nullable), 'append_to must be of type Nullable'
        ...
        if client_modified_utc is not None:
            assert isinstance(client_modified_utc, Nullable), 'client_modified_utc must be of type Nullable'
...
class ConflictReason(object):
    ...
    :
        ...
        Only one argument can be set.
                ...
                :param None folder: Conflict with a folder.
                :param None file: Conflict with a file.
                :param None autorename_failed: Could not autorename.
        """
        assert_only_one        ((
            folder=folder,
            file=file,
...
class ConflictError(object):
    ...
    :
        ...
        """
        if isinstance(reason, ConflictReason.Folder):
            self.reason = ConflictReason(folder=reason)
        if isinstance(reason, ConflictReason.File):
            self.reason = ConflictReason(file=reason)
        if isinstance(reason, ConflictReason.AutorenameFailed):
            self.reason = ConflictReason(autorename_failed=reason)
...
class UploadCommitError(object):
    ...
    :
        ...
        :type conflict: :class:`ConflictError`
                :param None no_write_permission: User does not have permission
                to write in the folder. An example of this is if the folder is a
                read-only shared folder.
                :param None insufficient_quota: User does not have sufficient
                space quota to save the file.
        """
        assert_only_one        ((
            conflict=conflict,
...
class BaseFiles(Namespace):
    ...
    :
        ...
        return         ((
            FileInfo.from_json(r.obj_segment),
            r.binary_segment,
        ))
    ...
    :
        ...
        o = None
        r = self._dropbox.request        ((Dropbox.Host.API_CONTENT,
          'files/UploadStart/upload/start',
    ...
    def upload_append_upload_append    ((
        self,
        f,
        upload_id,
        offset,
    ...
    :
        ...
        r = self._dropbox.request        ((Dropbox.Host.API_CONTENT,
          'files/UploadAppend/upload/append',
          Dropbox.RouteStyle.UPLOAD,
          o,
          f))
    ...
    :
        ...
        Error codes:
            conflict
                    no_write_permission: User does not have permission to write
                    in the folder. An example of this is if the folder is a
                    read-only shared folder.
                    insufficient_quota: User does not have sufficient space
